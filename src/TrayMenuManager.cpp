...
void TrayMenuManager::onAccountsListed(
    const QList<TailAccountInfo> &foundAccounts) {
...
void TrayMenuManager::buildConnectedMenu(const TailStatus &pTailStatus) {
  ...
  for (const auto &dev : pTailStatus.peers) {
    ...
    // TODO: Do we want a setting for showing them in a separate menu or
    // something?
    ...
    if (dev.id != pTailStatus.self.id) {
      ...
      if (!dev.online) {
        ...
        disposableConnectedMenuActions.push_back(deviceMenu->addSeparator());
        ...
        connect(sendFileAction, &QAction::triggered, this, [this, name](bool) {
          ...
          // to us
          pTailRunner->sendFile(name, file, new QString(file));
        });
        ...
        if (!scripts.isEmpty()) {
          ...
          for (const auto &script : scripts) {
            ...
            connect(fileAction, &QAction::triggered, this,
                    [fileInfo, ip, dnsName]() {
                      qDebug()
                          << "Running script: " << fileInfo.absoluteFilePath();
                      qint64 pid = 0;
                      auto success = QProcess::startDetached(
                          fileInfo.absoluteFilePath(), {ip, dnsName},
                          fileInfo.absolutePath(), &pid);
                      if (!success) {
                        qWarning() << "Failed to start script: "
  ...
  if (settings.tailDriveEnabled()) {
    ...
    if (!pTailStatus.drivesConfigured && pTailStatus.drives.count() < 1) {
      auto *action =
          drives->addAction(tr("Not configured, click to configure"));
      disposableConnectedMenuActions.push_back(action);
      connect(action, &QAction::triggered, this, [this](bool) {
        // Open help link in browser
        QDesktopServices::openUrl(
            QUrl("https://tailscale.com/kb/1369/taildrive"));
      });
    } else {
      ...
      connect(addAction, &QAction::triggered, this, [this](bool) {
        ManageDriveWindow wnd(TailDriveInfo{},
                              reinterpret_cast<QWidget *>(this->parent()));
      ...
      connect(mountAction, &QAction::triggered, this, [this](bool) {
        ...
        qDebug() << "Will try to mount " << fsType << " to local path "
                 << mountPath;
      ...
      if (pTailStatus.drives.count() > 0) {
        disposableConnectedMenuActions.push_back(drives->addSeparator());
      }
      ...
      for (const auto &drive : pTailStatus.drives) {
        ...
        connect(removeAction, &QAction::triggered, this,
                [this, drive](bool) { pTailRunner->removeDrive(drive); });

        disposableConnectedMenuActions.push_back(driveMenu->addSeparator());
        ...
        connect(openAction, &QAction::triggered, this, [this, drive](bool) {
          QDesktopServices::openUrl(
              QUrl::fromLocalFile(QDir::toNativeSeparators(drive.path)));
        });
      }
#endif
    }
  ...
  if (hasMullvadNodes) {
    disposableConnectedMenuActions.push_back(exitNodes->addSeparator());
  ...
  QAction *exitNodeAllowNwAccess =
      exitNodes->addAction(tr("Allow local network access"));
...
void TrayMenuManager::setupWellKnownActions() const {
  connect(pLoginAction.get(), &QAction::triggered, this,
          [this](bool) { pTailRunner->login(); });

  connect(pConnect.get(), &QAction::triggered, this,
          [this](bool) { pTailRunner->start(); });
  ...
  connect(pRefreshLocalDns.get(), &QAction::triggered, this,
          [this](bool) { pSysCommand->refreshDns(); });
}
